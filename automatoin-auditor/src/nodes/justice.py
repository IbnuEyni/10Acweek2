...
def chief_justice_node(state: AgentState) -> AgentState:
    ...
    for criterion_id, criterion_opinions in opinions_by_criterion.items():
        ...
        if variance > 2:
            ...
            # Low variance - weighted average favoring Tech Lead
            weighted_score = (
                prosecutor_op.score * 0.20 +
                defense_op.score * 0.15 +
                tech_lead_op.score * 0.65
            )
            final_scores[criterion_id] = round(weighted_score)
...
"""
    ...
    report += "3. **Weighted Resolution**: Tech Lead 75%, Prosecutor 15%, Defense 10% (high variance)\n"
    report += "4. **Weighted Resolution**: Tech Lead 65%, Prosecutor 20%, Defense 15% (low variance)\n\n"
